...
TODO
----
- console scripts (in plugin): add Grails Console scripts for simple Akka Remote Server, and related Client to ensure that all is good ...
- console scripts: copy them even in test webapps and ensure that all is good even there ...
- console scripts: add documentation on how to use them, and with evidence if they are needed by integration tests ...
- console scripts: in client test, get reference to remote actors via ActorIdentity messages ...
- console scripts: add another client test, for creating remote actors ...
- test server application using Akka microkernel (under tests/server): grails_akka_test.server.AkkaRemoteServerKernel.groovy, make it compile and run ...

- for 2.2.5 (maintenance and update docs and samples):
	- update dependencies ... ok

- for post-2.2.5:
	- create a maintenance branch for 2.2.x ...

- for 2.3.11_2.10 (update to latest Akka, the version for Scala-2.10.x):
* Later 2015
** 2.3.11_2.10 release
*** Updated to Akka-2.3.11 (aligned with Scala-2.10.4)

- create another maintenance branch for 2.3.x_2.10, and set 2.3.x_2.11 in master ...


- later:
	- update dependencies to latest Akka-2.3.x (but stay on Scala-2.10.x for now) ...
	- check if set a new plugin release as 2.3.x or 2.3.x_2.10 (like in the Scala world), and then make even a _2.11 release (and update plugin page with this info) ...
	- try to use traits (since Groovy-2.3) in messages and actors, at least in test webapps with Grails-2.4.x ...
